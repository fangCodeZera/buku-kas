...
| 2026-04-13 | Added PasswordChangeModal — detects Supabase PASSWORD_RECOVERY event in AuthContext.js, shows inline modal in App.js with new password + confirmation fields, validates min 6 chars + match, updates via supabase.auth.updateUser(). "Nanti Saja" skips the change and clears recovery state. | Completes the password reset flow |
