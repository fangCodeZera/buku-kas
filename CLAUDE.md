...
3. **Import is fully functional in Supabase mode.** `handleImport` is `async` — runs local migrations first (via `loadData()`), then upserts each entity (transactions, contacts, stockAdjustments, itemCatalog, itemCategories, settings) to Supabase via `Promise.allSettled`. The Settings.js import button is always enabled.
...
| 2026-04-13 | CSV export (Reports.js): currency columns now formatted with Indonesian thousand separators (`id-ID` locale via `fmtCSV()`). Renamed "Nilai Total" → "Total Transaksi" and "Nilai (Rp)" → "Sudah Dibayar". Formatted values are wrapped in double quotes by the existing `q()` helper — no CSV parsing issues. Non-currency columns (Karung, Berat, Stok) unchanged. | Improved readability of exported reports |
