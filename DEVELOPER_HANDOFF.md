...
6. **Catalog:** `itemCatalog[]` is canonical for item selection. `itemCategories[]` is for grouping/display. They are synced by `addCatalogItem` and `updateCatalogItem`. To rename a catalogued item with full cascade (catalog entry name + all subtype combined names on transactions + stock adjustments), use the "Ubah Nama" button on the base row — it calls `renameInventoryItem` in App.js which handles everything atomically in a single `update()` call. Never try to rename via the catalog edit form's Simpan button — `handleUpdateCatalogItem` no longer cascades name changes (only handles subtype/unit edits for paths reachable from `handleAddSubtype` and inline subtype rename).
