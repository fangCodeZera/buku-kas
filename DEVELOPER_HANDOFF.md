...
src/
  ...
  components/
    ...
    CategoryModal.js              ~640  Category management with drag-and-drop. Archive-aware filter: `showArchived` toggle hides archived pills + entire groups with 0 visible items; orphan keys (in neither active nor archived sets) always hidden with one-shot console.warn on mount. `itemCatalog` prop required for filter sets.
